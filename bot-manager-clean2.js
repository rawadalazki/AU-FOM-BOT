...
class TelegramBotService {
  ...
  async handleMessage(message) {
    ...
    if (text.startsWith('/search ') || text.startsWith('??? ') || text.startsWith('???? ')) {
      const query = text.replace(/^\/search\s+|^???\s+|^????\s+/i, '').trim();
    ...
    if (clickedMenu) {
      ...
      const unknownMsg = user.language === 'ar' ? '?????? ?? ???? ????. ?????? ?????? ?? ???????.' : 'Sorry, I did not understand that. Please select from the menu.';
  ...
  async processMenuClick(chatId, user, clickedMenu, allMenus) {
    ...
    if (clickedMenu.reply_type === 'submenu') {
      ...
      await this.apiCall('sendMessage', { 
        chat_id: chatId, 
        text: reply || (user.language === 'ar' ? '?? ???? ?????' : 'No content'),
  ...
  async handleDirectFileLink(chatId, menuId, lang) {
    ...
    if (!menu || menu.faculty_id !== this.facultyId || menu.reply_type !== 'file') {
      const err = lang === 'ar' ? '????? ??? ????? ?? ?? ????.' : 'File not found or deleted.';
  ...
  async searchFiles(chatId, query, lang) {
    if (query.length < 2) {
      await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ????? ????? ??? ?????.' : 'Please enter at least 2 characters.' });
      return;
    }
    ...
    if (rows.length === 0) {
      await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ??? ?????? ??? ?????.' : '? No results found.' });
      return;
    }

    let resultText = lang === 'ar' ? `?? ????? ????? ?? "${query}":\n\n` : `?? Search results for "${query}":\n\n`;
  ...
  async handleCallbackQuery(callbackQuery) {
    ...
    if (data.startsWith('lang_')) {
      ...
      if (action === 'rename') {
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ??? ?????' : '? File Deleted' });
  ...
  async handleAdminStateMessage(chatId, message, lang, state) {
    ...
    if (text === '?? ????? ???????' || text === '?? Cancel Operation') {
       if (state && state.menuId) {
         ...
         if (menu) {
           ...
           await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ???????.' : 'Action cancelled.', reply_markup: { remove_keyboard: true } });
    ...
    if (text === '? ?????' || text === '? Close') {
      await dbHelper.deleteAdminState(chatId);
      await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ????? ???? ??????.' : 'Admin panel closed.', reply_markup: { remove_keyboard: true } });
    ...
    if (state.action === 'admin_home') {
      if (text.includes('????? ???????') || text.includes('Manage Menus')) {
      ...
      } else if (text.includes('????? ????') || text.includes('Broadcast')) {
      ...
      } else if (text.includes('????????') || text.includes('Stats')) {
        const users = await dbHelper.getBotUsersByFaculty(this.facultyId, 'telegram');
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? `?? ???????? ?????:\n\n?????? ?????????: ${users.length}` : `?? Bot Stats:\n\nTotal Subscribers: ${users.length}` });
      } else if (text.includes('???????') || text.includes('Core Settings')) {
        await dbHelper.setAdminState(chatId, { action: 'managing_config' });
        const cfgText = lang === 'ar' 
          ? '?? ??????? ?????\n\n???? ??????? ???? ???? ???????:'
          : '?? Bot Configuration\n\nWhat would you like to edit?';
        const cfgKb = [
          [{ text: lang === 'ar' ? '?? ???????' : '?? Welcome Msg' }, { text: lang === 'ar' ? '?? ????? ???????' : '?? Maintenance Msg' }],
          [{ text: lang === 'ar' ? '? ????? ???? ??????' : '? Empty Button Msg' }, { text: lang === 'ar' ? '? ????? ?? ??? ?????' : '? Unknown Text Msg' }],
          [{ text: lang === 'ar' ? '?? ????? ?? ???? ???' : '?? No File Msg' }],
          [{ text: lang === 'ar' ? '?? ????? ????????' : '?? Toggle Monitoring' }],
          [{ text: lang === 'ar' ? '?? ????????' : '?? Home' }]
        ];
        await this.apiCall('sendMessage', { chat_id: chatId, text: cfgText, reply_markup: { keyboard: cfgKb, resize_keyboard: true } });
      } else if (text.includes('????? ????') || text.includes('Add Sub-Admin')) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_subadmin_id' });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ????? ?????? (ID) ????? ??????? ?????? ?????? ???? ??????:' : 'Please send the Telegram Chat ID of the new sub-admin:', reply_markup: { remove_keyboard: true }});
      }
      return;
    }
    ...
    if (state.action === 'managing_config') {
      if (text.includes('???????') || text.includes('Welcome Msg')) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_welcome_ar' });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ????? ??????? ??????? (???????):' : 'Send new welcome message (Arabic):', reply_markup: { remove_keyboard: true } });
      } else if (text.includes('????? ???????') || text.includes('Maintenance Msg')) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_disabled_msg_ar' });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ????? ??????? ??????? (???????):' : 'Send maintenance message (Arabic):', reply_markup: { remove_keyboard: true } });
      } else if (text.includes('????? ???? ??????') || text.includes('Empty Button Msg')) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_empty_msg_ar' });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??????? ???? ???? ??? ????? ??? ?? ?? ????? ??? ?? ???? (???????):' : 'Send new empty button message (Arabic):', reply_markup: { remove_keyboard: true } });
      } else if (text.includes('?? ??? ?????') || text.includes('Unknown Text Msg')) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_unknown_msg_ar' });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??????? ???? ???? ????? ???? ???????? ???? ?? ????? ????? (???????):' : 'Send message for unknown user input (Arabic):', reply_markup: { remove_keyboard: true } });
      } else if (text.includes('?? ???? ???') || text.includes('No File Msg')) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_no_file_msg_ar' });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??????? ???? ???? ????? ????? ???????? ??? ?? ??? ???? ????? ????? (???????):' : 'Send message when a file is missing (Arabic):', reply_markup: { remove_keyboard: true } });
      } else if (text.includes('????? ????????') || text.includes('Toggle Monitoring')) {
        ...
        const msg = newState 
            ? (lang === 'ar' ? '? ?? ????? ?????? ?????? ??????.' : '? Admin activity monitoring enabled.')
            : (lang === 'ar' ? '? ?? ????? ?????? ??????.' : '? Admin activity monitoring disabled.');
        await this.apiCall('sendMessage', { chat_id: chatId, text: msg });
      }
      return;
    }
    ...
    if (state.action === 'managing_menus') {
      ...
      if (text.includes('??????? ??????') || text.includes('Parent Menu')) {
      ...
      if (state.viewingMenuDetailsId) {
        ...
        if (text.includes('???? ??????? ???????') || text.includes('Back to Parent')) {
          const menu = await dbHelper.getMenuById(menuId);
          const targetId = menu ? menu.parent_id : null;
          await dbHelper.setAdminState(chatId, { action: 'managing_menus', currentMenuId: targetId, viewingMenuDetailsId: null });
          await this.sendAdminReplyMenus(chatId, targetId, lang);
        } else if (text.includes('????? ???????') || text.includes('Rename')) {
          await dbHelper.setAdminState(chatId, { action: 'awaiting_rename_title_ar', menuId });
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ????? ?????? ???? (?????? ???????):' : 'Send the new name in Arabic:', reply_markup: { remove_keyboard: true } });
        } else if (text.includes('????? ???????') || text.includes('Edit Content')) {
          const menu = await dbHelper.getMenuById(menuId);
          if (menu.reply_type === 'submenu') {
             await dbHelper.setAdminState(chatId, { action: 'awaiting_edit_submenu_ar', menuId });
             await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??????? ????????? ???? ????? ??? ???? ??? ??????? (???????):' : 'Send the prompt message for this submenu (Arabic):', reply_markup: { remove_keyboard: true } });
          } else if (menu.reply_type === 'text') {
             await dbHelper.setAdminState(chatId, { action: 'awaiting_edit_text_ar', menuId });
             await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ???? ?????? (???????):' : 'Send the new text (Arabic):', reply_markup: { remove_keyboard: true } });
          } else if (menu.reply_type === 'file') {
             await dbHelper.setAdminState(chatId, { action: 'awaiting_edit_file_doc', menuId });
             await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ????? ??????. ?????? ????? ????? ?????? ??????? ???:' : 'Send the new file, with optional Arabic caption:', reply_markup: { remove_keyboard: true } });
          }
        } else if (text.includes('??????? ?????') || text.includes('Replace File')) {
          await dbHelper.setAdminState(chatId, { action: 'awaiting_edit_file_doc', menuId });
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ????? ?????? ????? ?? ???? /skip ?????? ????? ???:' : 'Send the new file, or /skip to only edit caption:', reply_markup: { remove_keyboard: true } });
        } else if (text.includes('????? ?????') || text.includes('Inline Buttons')) {
          await dbHelper.setAdminState(chatId, { action: 'awaiting_inline_btn', menuId });
          const m = lang === 'ar' 
            ? '????? ?? ???? (????)\n\n???? ??? ???? ??????? ??????? ????? (-)\n????: `???? ??????? - https://example.com`\n?? ???? /clear ???? ??????? ???????.' 
            : 'Add Inline Button (URL)\n\nSend title and URL separated by hyphen (-)\nExample: `Website - https://example.com`\nOr send /clear to remove all.';
          await this.apiCall('sendMessage', { chat_id: chatId, text: m, parse_mode: 'Markdown', reply_markup: { remove_keyboard: true } });
        } else if (text.includes('????? ???????') || text.includes('Change Order')) {
          ...
          await this.sendAdminMoveOrder(chatId, menuId, lang);
        } else if (text.includes('??? ??? ????? ????') || text.includes('Move Menu')) {
          await dbHelper.setAdminState(chatId, { action: 'awaiting_move_target', menuId });
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??? ID ????? ???????? ????? (???? null ????? ??????? ????????):' : 'Send target Menu ID (or null for root):', reply_markup: { remove_keyboard: true } });
        } else if (text.includes('??? ????') || text.includes('Delete Button')) {
          const menu = await dbHelper.getMenuById(menuId);
          if (menu) {
            const parentId = menu.parent_id;
            await dbHelper.deleteMenu(menuId);
            await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ????? ?????.' : 'Deleted successfully.' });
      ...
      if (text === '?') {
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ??? ???? ???????' : 'What type of button?', reply_markup: {
          keyboard: [
            [{ text: lang === 'ar' ? '?? ?? ?????' : '?? Menu Button' }],
            [{ text: lang === 'ar' ? '?? ?? ???' : '?? File Button' }],
            [{ text: lang === 'ar' ? '?? ?? ??' : '?? Text Button' }],
            [{ text: lang === 'ar' ? '? ?????' : '? Cancel' }]
    ...
    if (state.action === 'managing_menus_add_type') {
      if (text.includes('?????') || text.includes('Cancel')) {
      ...
      let type = null;
      if (text.includes('?????') || text.includes('Folder')) type = 'submenu';
      else if (text.includes('?????') || text.includes('File')) type = 'file';
      else if (text.includes('???') || text.includes('Text')) type = 'text';

      if (type) {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_newmenu_title_ar', currentMenuId: state.currentMenuId, newType: type });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??? ???? ?????? (????????):' : 'Send the new button name (Arabic):', reply_markup: { remove_keyboard: true } });
      }
      return;
    }

    if (state.action === 'managing_menus_move_order') {
      if (text.includes('????? ???????') || text.includes('Cancel Order')) {
      ...
      let direction = null;
      if (text.includes('Up') || text.includes('??')) direction = 'up';
      if (text.includes('Down') || text.includes('??')) direction = 'down';
    ...
    switch (state.action) {
      case 'awaiting_subadmin_id':
        if (!/^\d+$/.test(text)) {
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ??? ????. ??? ?? ????? ??? ????? ???.' : 'Invalid ID. Must contain only digits.' });
          return;
        }
        ...
        if (currentAdmins.includes(text)) {
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '??? ?????? ????? ??????.' : 'This ID already exists.' });
        } else {
      ...
      case 'awaiting_announcement_title_ar':
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????. ???? ???? ????? ??????? ???????? (???????):' : 'Great. Now send the detailed content (Arabic):' });
        break;

      case 'awaiting_announcement_content_ar':
        state.contentAr = text;
        const statusAnnMsg = await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ???? ????? ??????? ????????...' : '?? Translating announcement...' });
        ...
        await this.apiCall('sendMessage', { 
          chat_id: chatId, 
          text: lang === 'ar' ? '??? ???????. ??? ??? ???? ??? ???? ??????? ????? ????? ?? ???? /skip ????? ????? ??? ??? ??????? ??????.' : 'Translated. Send an attachment file now, or /skip to broadcast without a file.'
        });
        break;
      ...
      case 'awaiting_announcement_file':
        if (text === '/skip') {
          ...
          if (doc) {
            ...
            await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ?? ????? ?? ?????? ??? ?????. ?????? ??????? ?? Document ?? ???? /skip.' : 'File not recognized. Send as Document or /skip.' });
          }
        }
        break;
      ...
      case 'awaiting_rename_title_ar':
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ??? ????? ??????? ?????!' : '? Renamed!' });
      ...
      case 'awaiting_edit_submenu_ar':
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ????? ??????? ?????????!' : '? Prompt message updated!' });
        await this.sendAdminMenuDetails(chatId, state.menuId, lang);
        break;
      ...
      case 'awaiting_edit_text_ar':
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ????? ????!' : '? Text updated!' });
        await this.sendAdminMenuDetails(chatId, state.menuId, lang);
        break;
      ...
      case 'awaiting_replace_file_doc':
        ...
        if (docReplace) {
          try {
            ...
            const doneBtn = lang === 'ar' ? '? ??' : '? Done';
            await this.apiCall('sendMessage', { 
              chat_id: chatId, 
              text: lang === 'ar' ? '? ?? ??????? ??????? ??????? ????? ?????. ???? ?????? ?? ???? "??":' : '? Files replaced. Send more or press "Done":',
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ????? ???.' : 'Please send a document.' });
        }
        break;

      case 'awaiting_edit_file_doc':
        if (text === '/skip' || text === '? Done' || text === '? ??' || text === '/done') {
          state.action = 'awaiting_edit_file_cap_ar';
          await dbHelper.setAdminState(chatId, state);
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '???? ????? ?????? ????? (???????) ?? /skip ???????? ???????:' : 'Send new Arabic caption, or /skip to keep current:', reply_markup: { remove_keyboard: true } });
        } else {
          ...
          if (doc2) {
            ...
            await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ????? ??? (Document) ?? ????? ??? "??".' : 'Please send a document or press "Done".' });
          }
        }
        break;
      ...
      case 'awaiting_edit_file_cap_ar':
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ????? ?????!' : '? Saved successfully!', reply_markup: { remove_keyboard: true } });
        await this.sendAdminMenuDetails(chatId, state.menuId, lang);
        break;
      ...
      case 'awaiting_move_target':
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ????? ?????!' : '? Moved successfully!' });
        await this.sendAdminMenuDetails(chatId, state.menuId, lang);
        break;
      ...
      case 'awaiting_newmenu_title_ar':
        ...
        if (state.newType === 'submenu') {
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ??? ????? ???????! ????? ?????? ????? ???? ?????? ????? ?????.' : '? Submenu added!' });
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ??? ?????. ???? ???? ??????? ????? ???? (???????):' : '? Name saved. Send text content (Arabic):' });
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ??? ?????. ???? ???? ????? ?????? (Document):' : '? Name saved. Send the file document:' });
        }
        break;
      ...
      case 'awaiting_welcome_ar':
        ...
        await dbHelper.deleteAdminState(chatId);
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ???????!' : '? Updated!' });
        await this.sendAdminHome(chatId, lang);
        break;
      ...
      case 'awaiting_inline_btn':
        ...
        if (text === '/clear') {
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ??? ??????? ???????.' : 'Inline buttons cleared.' });
        ...
        if (btnParts.length < 2) {
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '????? ????. ??????: ????? - ??????' : 'Invalid format. Use: Title - URL' });
          break;
        }
        ...
        if (!bUrl.startsWith('http')) {
           await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ??? ?? ???? ?? http ?? https' : 'URL must start with http or https' });
           break;
        }
        ...
        await dbHelper.setAdminState(chatId, { action: 'managing_menus', currentMenuId: menuBtn.parent_id, viewingMenuDetailsId: menuBtn.id });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ????' : '? Added' });
        await this.sendAdminMenuDetails(chatId, menuBtn.id, lang);
        break;
      ...
      default:
        ...
        await dbHelper.deleteAdminState(chatId);
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ????? ?????? ????????.' : 'Admin session closed.', reply_markup: { remove_keyboard: true } });
        break;
    }
  }
  ...
  async handleAdminAnnouncementBroadcast(chatId, state, doc, lang) {
    ...
    const stMsg = await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?? ???? ????...' : '?? Broadcasting...' });
    ...
    try {
      ...
      await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ????!' : '? Broadcasted!' });
      await this.sendAdminHome(chatId, lang);
    } catch(e) {
  ...
  async handleAdminAddFileButton(chatId, state, doc, lang) {
    try {
      ...
      await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '? ?? ????? ?? ????? ?????!' : '? File button added!' });
  ...
  async sendAdminHome(chatId, lang) {
    ...
    const keyboard = [
      [{ text: lang === 'ar' ? '?? ????? ??????? ????????' : '?? Manage Menus & Files' }],
      [{ text: lang === 'ar' ? '?? ????? ????? ????' : '?? Broadcast Announcement' },
       { text: lang === 'ar' ? '?? ???????? ?????' : '?? Bot Statistics' }]
    ];
    
    if (isCentralAdmin) {
      keyboard.push([
        { text: lang === 'ar' ? '?? ??????? ????? ????????' : '?? Core Settings' },
        { text: lang === 'ar' ? '? ????? ???? ????' : '? Add Sub-Admin' }
      ]);
    }
    
    keyboard.push([{ text: lang === 'ar' ? '? ?????' : '? Close' }]);

    await this.apiCall('sendMessage', { 
      chat_id: chatId, 
      text: lang === 'ar' ? '??? ???? ??????:' : '??? Admin Panel:', 
  ...
  async sendAdminReplyMenus(chatId, parentId, lang) {
    ...
    let txt = lang === 'ar' ? '?? ??????? ???????:' : '?? Current Menus:';
    if (parentId !== null) {
      const pMenu = menus.find(m => m.id === parentId);
      txt = lang === 'ar' ? `?? ????: ${pMenu.title_ar}` : `?? Inside: ${pMenu.title_en}`;
    }
    ...
    if (parentId !== null) {
      kb.push([{ text: lang === 'ar' ? '?? ??????? ??????' : '?? Parent Menu' }]);
    }
    kb.push([{ text: lang === 'ar' ? '?? ????????' : '?? Home' }]);
  ...
  async sendAdminMenuDetails(chatId, menuId, lang) {
    ...
    kb.push([
      { text: lang === 'ar' ? '?? ????? ???????' : '?? Rename', callback_data: "admin_rename_" + menuId },
      { text: lang === 'ar' ? '??? ??? ????' : '??? Delete Button', callback_data: "admin_delbtn_" + menuId }
    ]);

    // Type specific actions
    if (menu.reply_type === 'submenu') {
      kb.push([{ text: lang === 'ar' ? '?? ??? ??????' : '?? Open Folder', callback_data: "admin_open_" + menuId }]);
    } else if (menu.reply_type === 'file') {
      kb.push([{ text: lang === 'ar' ? '??? ??? ???????' : '??? Delete Content', callback_data: "admin_delcontent_" + menuId }]);
      kb.push([
        { text: lang === 'ar' ? '??? ?????? ???????' : '??? Preview Files', callback_data: "admin_previewfiles_" + menuId },
        { text: lang === 'ar' ? '? ????? ???' : '? Add File', callback_data: "admin_addfile_" + menuId }
      ]);
    } else if (menu.reply_type === 'text') {
      kb.push([{ text: lang === 'ar' ? '??? ??? ???????' : '??? Delete Content', callback_data: "admin_delcontent_" + menuId }]);
      kb.push([{ text: lang === 'ar' ? '?? ????? ????' : '?? Edit Text', callback_data: "admin_edittext_" + menuId }]);
    }

    // Advanced options (Inline buttons, Move, Order)
    kb.push([
      { text: lang === 'ar' ? '?? ????? ?????' : '?? Inline Buttons', callback_data: "admin_inline_" + menuId },
      { text: lang === 'ar' ? '?? ???' : '?? Move', callback_data: "admin_move_" + menuId }
    ]);

    kb.push([{ text: lang === 'ar' ? '?? ????? ???????' : '?? Change Order', callback_data: "admin_order_" + menuId }]);
  ...
  async sendFilePage(chatId, menuId, page, lang, caption = null) {
    ...
    if (!allFiles || allFiles.length === 0) {
      ...
      if (menu && menu.telegram_file_id) {
        try {
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '??? ?? ???????' : 'Error sending file' });
        }
      } else {
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '?????? ?? ???? ??? ????.' : 'Sorry, no file attached.' });
      }
      return;
    }
    ...
    if (hasError) {
      await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? '??? ?? ????? ??? ???????.' : 'Error sending some files.' });
    }

    // Show pagination controls when there are multiple pages
    if (totalPages > 1) {
      const pageLabel = lang === 'ar'
        ? `?? ???? ${page + 1} ?? ${totalPages} (${totalFiles} ???)`
        : `?? Page ${page + 1} of ${totalPages} (${totalFiles} files)`;
      const kb = [];
      const navRow = [];
      if (page > 0) {
        navRow.push({ text: lang === 'ar' ? '?? ??????' : '?? Previous', callback_data: `fp_${menuId}_${page - 1}` });
      }
      if (page < totalPages - 1) {
        navRow.push({ text: lang === 'ar' ? '?????? ??' : 'Next ??', callback_data: `fp_${menuId}_${page + 1}` });
      }
      if (navRow.length > 0) kb.push(navRow);
      kb.push([{ text: lang === 'ar' ? '? ?????' : '? Exit', callback_data: `fe_${menuId}` }]);
  ...
  async translateArToEn(text) {
    const dict = {
      '?? ??????': 'About the Faculty', '??????': 'Faculty', '???????': 'Departments', '???????': 'Files',
      '?????????': 'Documents', '????? ????': 'Contact Us', '???????': 'Announcements', '???? ??????': 'Student Affairs'
    };
    const t = text.trim();
