...
class TelegramBotService {
  ...
  async handleAdminStateMessage(chatId, message, lang, state) {
    ...
    if (state.action === 'managing_menus') {
      ...
      if ($state.viewingMenuDetailsId) {
        ...
        if (text.includes('فتح المجلد') || text.includes('Open Folder')) {
  ...
  async sendAdminMenuDetails(chatId, menuId, lang) {
    ...
    let txt = lang === 'ar' ? `ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ø²Ø±: ${title}\nØ§Ù„Ù†ÙˆØ¹: ${menu.reply_type}` : `Button Details: ${title}\nType: ${menu.reply_type}`;
    if (menu.reply_type === 'file') {
      txt += `\nFile: ${menu.file_name}`;
    }
    
    const kb = [
      [{ text: lang === 'ar' ? 'âœï¸ Ø¥Ø¹Ø§Ø¯Ø© Ø§Ù„ØªØ³Ù…ÙŠØ©' : 'âœï¸ Rename' }, { text: lang === 'ar' ? 'ðŸ“ ØªØ¹Ø¯ÙŠÙ„ Ø§Ù„Ù…Ø­ØªÙˆÙ‰' : 'ðŸ“ Edit Content' }]
    ];

    if (menu.reply_type === 'submenu') {
      kb.unshift([{ text: lang === 'ar' ? 'ðŸ“‚ ÙØªØ­ Ø§Ù„Ù…Ø¬Ù„Ø¯' : 'ðŸ“‚ Open Folder' }]);
    }

    if (menu.reply_type === 'file') {
      kb.push([{ text: lang === 'ar' ? 'ðŸ“Ž Ø§Ø³ØªØ¨Ø¯Ø§Ù„ Ø§Ù„Ù…Ù„Ù' : 'ðŸ“Ž Replace File' }]);
    }
    
    if (menu.reply_type === 'text') {
      kb.push([{ text: lang === 'ar' ? 'ðŸ”— Ø¥Ø¶Ø§ÙØ© Ø£Ø²Ø±Ø§Ø± Ø´ÙØ§ÙØ©' : 'ðŸ”— Add Inline Buttons' }]);
    }

    kb.push([
      { text: lang === 'ar' ? 'â†•ï¸ ØªØºÙŠÙŠØ± Ø§Ù„ØªØ±ØªÙŠØ¨' : 'â†•ï¸ Change Order' },
      { text: lang === 'ar' ? 'ðŸ”„ Ù†Ù‚Ù„ Ø¥Ù„Ù‰ Ù‚Ø§Ø¦Ù…Ø© Ø£Ø®Ø±Ù‰' : 'ðŸ”„ Move Menu' }
    ]);
    kb.push([{ text: lang === 'ar' ? 'ðŸ—‘ï¸ Ø­Ø°Ù Ø§Ù„Ø²Ø±' : 'ðŸ—‘ï¸ Delete Button' }]);
    kb.push([{ text: lang === 'ar' ? 'â¬…ï¸ Ø¹ÙˆØ¯Ø© Ù„Ù„Ù‚Ø§Ø¦Ù…Ø© Ø§Ù„Ø³Ø§Ø¨Ù‚Ø©' : 'â¬…ï¸ Back to Parent' }]);
    ...
    if (!allFiles || allFiles.length === 0) {
      ...
      if (menu && menu.telegram_file_id) {
        try {
          ...
          await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'Ø®Ø·Ø£ ÙÙŠ Ø¥Ø±Ø³Ø§Ù„ Ø§Ù„Ù…Ù„Ù' : 'Error sending file' });
        }
      } else {
