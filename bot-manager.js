...
class TelegramBotService {
      ...
      for (const adminId of adminIds) {
        if (adminId) {
          ...
          await this.apiCall('sendMessage', { 
            chat_id: adminId, 
            text: `ðŸ‘€ **Ù…Ø±Ø§Ù‚Ø¨Ø© Ø§Ù„Ù†Ø´Ø§Ø·**\n\nðŸ‘¤ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…: ${userStr} (ID: ${message.from.id})\nðŸ’¬ Ø§Ù„Ù†Øµ: ${message.text || ''}`,
            parse_mode: 'Markdown'
          });
        }
      }
  ...
  async handleCallbackQuery(callbackQuery) {
    ...
    if (data.startsWith('lang_')) {
      ...
      const cancelKb = { keyboard: [[{ text: lang === 'ar' ? 'ðŸš« Ø¥Ù„ØºØ§Ø¡ Ø§Ù„Ø¹Ù…Ù„ÙŠØ©' : 'ðŸš« Cancel Operation' }]], resize_keyboard: true };

      await this.apiCall('deleteMessage', { chat_id: chatId, message_id: callbackQuery.message.message_id }).catch(() => {});

      if (action === 'rename') {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_rename_title_ar', menuId });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'Ø£Ø±Ø³Ù„ Ø§Ù„Ø§Ø³Ù… Ø§Ù„Ø¬Ø¯ÙŠØ¯ Ù„Ù„Ø²Ø± (Ø¨Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©):' : 'Send the new name in Arabic:', reply_markup: cancelKb });
      } else if (action === 'delbtn') {
        const menu = await dbHelper.getMenuById(menuId);
        await dbHelper.deleteMenu(menuId);
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'âœ… ØªÙ… Ø­Ø°Ù Ø§Ù„Ø²Ø±' : 'âœ… Button Deleted' });
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'âœ… ØªÙ… Ø­Ø°Ù Ø§Ù„Ù…Ø­ØªÙˆÙ‰' : 'âœ… Content Deleted' });
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'âœ… ØªÙ… Ø­Ø°Ù Ø§Ù„Ù…Ù„Ù' : 'âœ… File Deleted' });
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'Ø£Ø±Ø³Ù„ Ø§Ù„Ù…Ù„Ù Ø§Ù„Ø¬Ø¯ÙŠØ¯. ÙˆÙŠÙ…ÙƒÙ†Ùƒ Ø¥Ø±ÙØ§Ù‚ Ø§Ù„Ø´Ø±Ø­ Ø¨Ø§Ù„Ù„ØºØ© Ø§Ù„Ø¹Ø±Ø¨ÙŠØ© Ù…Ø¹Ù‡:' : 'Send the new file, with optional Arabic caption:', reply_markup: cancelKb });
      } else if (action === 'edittext') {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_edit_text_ar', menuId });
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'Ø£Ø±Ø³Ù„ Ø§Ù„Ù†Øµ Ø§Ù„Ø¬Ø¯ÙŠØ¯ (Ø¨Ø§Ù„Ø¹Ø±Ø¨ÙŠ):' : 'Send the new text (Arabic):', reply_markup: cancelKb });
      } else if (action === 'inline') {
        await dbHelper.setAdminState(chatId, { action: 'awaiting_inline_btn', menuId });
        const m = lang === 'ar' 
          ? 'ðŸ”— Ø¥Ø¶Ø§ÙØ© Ø²Ø± Ø´ÙØ§Ù (Ø±Ø§Ø¨Ø·)\n\nØ£Ø±Ø³Ù„ Ø§Ø³Ù… Ø§Ù„Ø²Ø± ÙˆØ§Ù„Ø±Ø§Ø¨Ø· Ù…ÙØµÙˆÙ„ÙŠÙ† Ø¨Ø´Ø±Ø·Ø© (-)\nÙ…Ø«Ø§Ù„: Ù…ÙˆÙ‚Ø¹ Ø§Ù„Ø¬Ø§Ù…Ø¹Ø© - https://example.com\nØ£Ùˆ Ø£Ø±Ø³Ù„ /clear Ù„Ù…Ø³Ø­ Ø§Ù„Ø£Ø²Ø±Ø§Ø± Ø§Ù„Ø­Ø§Ù„ÙŠØ©.' 
          : 'ðŸ”— Add Inline Button (URL)\n\nSend title and URL separated by hyphen (-)\nExample: Website - https://example.com\nOr send /clear to remove all.';
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'Ø£Ø±Ø³Ù„ Ø§Ù„Ù€ ID Ø§Ù„Ø®Ø§Øµ Ø¨Ø§Ù„Ù‚Ø§Ø¦Ù…Ø© Ø§Ù„Ù‡Ø¯Ù (Ø§ÙƒØªØ¨ null Ù„Ù†Ù‚Ù„Ù‡ Ù„Ù„Ù‚Ø§Ø¦Ù…Ø© Ø§Ù„Ø±Ø¦ÙŠØ³ÙŠØ©):' : 'Send target Menu ID (or null for root):', reply_markup: cancelKb });
  ...
  async handleAdminStateMessage(chatId, message, lang, state) {
    ...
    if (text === 'ðŸš« Ø¥Ù„ØºØ§Ø¡ Ø§Ù„Ø¹Ù…Ù„ÙŠØ©' || text === 'ðŸš« Cancel Operation') {
    ...
    if (state.action === 'admin_home') {
      if (text.includes('Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù‚ÙˆØ§Ø¦Ù…') || text.includes('Manage Menus')) {
        ...
        const cfgKb = [
          ...
          [{ text: lang === 'ar' ? 'ðŸ‘€ Ø¥Ø¸Ù‡Ø§Ø± Ø§Ù„Ù…Ø±Ø§Ù‚Ø¨Ø©' : 'ðŸ‘€ Toggle Monitoring' }],
    ...
    if (state.action === 'managing_config') {
      if (text.includes('Ø§Ù„ØªØ±Ø­ÙŠØ¨') || text.includes('Welcome Msg')) {
      ...
      } else if (text.includes('Ø¥Ø¸Ù‡Ø§Ø± Ø§Ù„Ù…Ø±Ø§Ù‚Ø¨Ø©') || text.includes('Toggle Monitoring')) {
        ...
        const msg = newState 
            ? (lang === 'ar' ? 'âœ… ØªÙ… ØªÙØ¹ÙŠÙ„ Ù…Ø±Ø§Ù‚Ø¨Ø© Ø§Ù„Ù†Ø´Ø§Ø· Ù„Ù„Ø£Ø¯Ù…Ù†.' : 'âœ… Admin activity monitoring enabled.')
            : (lang === 'ar' ? 'âŒ ØªÙ… Ø¥ÙŠÙ‚Ø§Ù Ù…Ø±Ø§Ù‚Ø¨Ø© Ø§Ù„Ù†Ø´Ø§Ø·.' : 'âŒ Admin activity monitoring disabled.');
        await this.apiCall('sendMessage', { chat_id: chatId, text: msg });
      }
      return;
    }
    ...
    if (state.action === 'managing_menus') {
      ...
      const cleanText = text.replace(/^(ðŸ“|ðŸ“„|ðŸ“)\s*/, '').trim();
      const menus = await dbHelper.getMenusByFaculty(this.facultyId);
      ...
      if (text === 'âž•') {
        ...
        await this.apiCall('sendMessage', { chat_id: chatId, text: lang === 'ar' ? 'Ù…Ø§ Ù†ÙˆØ¹ Ø§Ù„Ø²Ø± Ø§Ù„Ø¬Ø¯ÙŠØ¯ØŸ' : 'What type of button?', reply_markup: {
          keyboard: [
            [{ text: lang === 'ar' ? 'ðŸ“ Ø²Ø± Ù‚Ø§Ø¦Ù…Ø©' : 'ðŸ“ Menu Button' }],
            [{ text: lang === 'ar' ? 'ðŸ“„ Ø²Ø± Ù…Ù„Ù' : 'ðŸ“„ File Button' }],
            [{ text: lang === 'ar' ? 'ðŸ“ Ø²Ø± Ù†Øµ' : 'ðŸ“ Text Button' }],
            [{ text: lang === 'ar' ? 'âŒ Ø¥Ù„ØºØ§Ø¡' : 'âŒ Cancel' }]
  ...
  async sendAdminMenuDetails(chatId, menuId, lang) {
    ...
    const typeStr = menu.reply_type === 'submenu' ? (lang === 'ar' ? 'ðŸ“ Ù…Ø¬Ù„Ø¯ (Ù‚Ø§Ø¦Ù…Ø©)' : 'Folder') :
                    menu.reply_type === 'file' ? (lang === 'ar' ? 'ðŸ“‘ Ù…Ù„Ù' : 'Files') :
                    (lang === 'ar' ? 'ðŸ“ Ù†Øµ' : 'Text');

    let txt = lang === 'ar' 
      ? "<b>ðŸ“‹ ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ø²Ø±:</b>\n\n<b>Ø§Ù„Ø§Ø³Ù…:</b> \n<b>Ø§Ù„Ù†ÙˆØ¹:</b> "
      : "<b>ðŸ“‹ Button Details:</b>\n\n<b>Name:</b> \n<b>Type:</b> ";

    const kb = [];
    
    // Core actions for all types
    kb.push([
      { text: lang === 'ar' ? 'âœï¸ Ø¥Ø¹Ø§Ø¯Ø© Ø§Ù„ØªØ³Ù…ÙŠØ©' : 'âœï¸ Rename', callback_data: "admin_rename_" + menuId },
      { text: lang === 'ar' ? 'ðŸ—‘ï¸ Ø­Ø°Ù Ø§Ù„Ø²Ø±' : 'ðŸ—‘ï¸ Delete Button', callback_data: "admin_delbtn_" + menuId }
    ]);

    // Type specific actions
    if (menu.reply_type === 'submenu') {
      kb.push([{ text: lang === 'ar' ? 'ðŸ“‚ ÙØªØ­ Ø§Ù„Ù…Ø¬Ù„Ø¯' : 'ðŸ“‚ Open Folder', callback_data: "admin_open_" + menuId }]);
    } else if (menu.reply_type === 'file') {
      kb.push([{ text: lang === 'ar' ? 'ðŸ—‘ï¸ Ø­Ø°Ù Ø§Ù„Ù…Ø­ØªÙˆÙ‰' : 'ðŸ—‘ï¸ Delete Content', callback_data: "admin_delcontent_" + menuId }]);
      kb.push([
        { text: lang === 'ar' ? 'ðŸ‘ï¸ Ù…Ø¹Ø§ÙŠÙ†Ø© Ø§Ù„Ù…Ù„ÙØ§Øª' : 'ðŸ‘ï¸ Preview Files', callback_data: "admin_previewfiles_" + menuId },
        { text: lang === 'ar' ? 'âž• Ø¥Ø¶Ø§ÙØ© Ù…Ù„Ù' : 'âž• Add File', callback_data: "admin_addfile_" + menuId }
      ]);
    } else if (menu.reply_type === 'text') {
      kb.push([{ text: lang === 'ar' ? 'ðŸ—‘ï¸ Ø­Ø°Ù Ø§Ù„Ù…Ø­ØªÙˆÙ‰' : 'ðŸ—‘ï¸ Delete Content', callback_data: "admin_delcontent_" + menuId }]);
      kb.push([{ text: lang === 'ar' ? 'ðŸ“ ØªØ¹Ø¯ÙŠÙ„ Ø§Ù„Ù†Øµ' : 'ðŸ“ Edit Text', callback_data: "admin_edittext_" + menuId }]);
    }

    // Advanced options (Inline buttons, Move, Order)
    kb.push([
      { text: lang === 'ar' ? 'ðŸ”— Ø£Ø²Ø±Ø§Ø± Ø´ÙØ§ÙØ©' : 'ðŸ”— Inline Buttons', callback_data: "admin_inline_" + menuId },
      { text: lang === 'ar' ? 'ðŸ”„ Ù†Ù‚Ù„' : 'ðŸ”„ Move', callback_data: "admin_move_" + menuId }
    ]);

    kb.push([{ text: lang === 'ar' ? 'â†•ï¸ ØªØºÙŠÙŠØ± Ø§Ù„ØªØ±ØªÙŠØ¨' : 'â†•ï¸ Change Order', callback_data: "admin_order_" + menuId }]);
    ...
    const toggleActiveStr = isActive
      ? (lang === 'ar' ? 'ðŸ”´ Ø¥ÙŠÙ‚Ø§Ù Ø§Ù„Ø²Ø±' : 'ðŸ”´ Disable') 
      : (lang === 'ar' ? 'ðŸŸ¢ ØªØ´ØºÙŠÙ„ Ø§Ù„Ø²Ø±' : 'ðŸŸ¢ Enable');
    const toggleHiddenStr = isHidden
      ? (lang === 'ar' ? 'ðŸ‘ï¸ Ø¥Ø¸Ù‡Ø§Ø± Ø§Ù„Ø²Ø±' : 'ðŸ‘ï¸ Show Button')
      : (lang === 'ar' ? 'ðŸš« Ø¥Ø®ÙØ§Ø¡ Ø§Ù„Ø²Ø±' : 'ðŸš« Hide Button');
  ...
  async sendFilePage(chatId, menuId, page, lang, caption = null, isAdminPreview = false) {
    ...
    for (let i = 0; i < pageFiles.length; i++) {
      ...
      if (isAdminPreview) {
        replyMarkup = {
          inline_keyboard: [[
             { text: lang === 'ar' ? '🗑️ حذف هذا الملف' : '🗑️ Delete this file', callback_data: "admin_delsinglefile_" + menuId + "_" + file.id }
          ]]
        };
      }
      try {
